  ...
  methods: {
    ...
    submitOrder(event) {
      ...
      App._post(event.currentTarget.dataset.tag == 'save' ? App.UrlList.GroupOrderSave : App.UrlList.GroupOrderSubmit, {
        ...
        if (_this.data.nextStep && event.currentTarget.dataset.tag == 'save') {
          setTimeout(() => { _this.jumpOrder(_this.data.orderTypes) }, 1000)
        } else if (event.currentTarget.dataset.tag == 'submit') {
          const pages = getCurrentPages();
          _this.navigateBack(pages, 'pages/orderList/index')
        }
      })
    },
